...
namespace IIASA.GeoTrees.HealthChecks;

public static class HealthChecksBuilderExtensions
{
    public static void AddGeoTreesHealthChecks(this IServiceCollection services)
    {
        ...
        if (!Uri.IsWellFormedUriString(healthCheckUrl, UriKind.Absolute))
        {
            ...
            var localUrl = urls.Split(';')
                .First()
                .Replace("+", "localhost")
                .Replace("*", "localhost")
                .Replace("[::]", "localhost");
            healthCheckUrl = localUrl.TrimEnd('/') + healthCheckUrl;
        }
        ...
        {
            settings.AddHealthCheckEndpoint("GeoTrees Health Status", healthCheckUrl);
        });
